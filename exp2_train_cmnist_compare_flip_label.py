...
class SimpleMLP(nn.Module):
    def __init__(self, num_classes, input_size=32):
        ...
        # First hidden layer
        self.layer1 = nn.Linear(3 * input_size * input_size, 128 * 4)
        self.bn1 = nn.BatchNorm1d(128 * 4)
        self.act1 = nn.ReLU()

        # Second hidden layer
        self.layer2 = nn.Linear(128 * 4, 128 * 2)
        self.bn2 = nn.BatchNorm1d(128 * 2)
        self.act2 = nn.ReLU()

        # Third hidden layer
        self.layer3 = nn.Linear(128 * 2, 64)
...
if __name__ == "__main__":
    ...
    results.append({"test_acc": test_acc, "test_loss": test_loss, "color": red_color, "color_cosine_similarity": 1.0,
    ...
    for sample_id in range(100):
        ...
        cosine_similarity_value = np.sum(np.array(red_color) * np.array(random_color)) / (
                    np.linalg.norm(red_color) * np.linalg.norm(random_color))

        # Load datasets with custom labels and colors
        # Load datasets with custom labels and colors
        train_set = CustomMNIST(root='./data', train=True, download=True, color=random_color, flip_label=True)
        test_set = CustomMNIST(root='./data', train=False, download=True, color=random_color, flip_label=True)
        ...
        results.append({"test_acc": test_acc, "test_loss": test_loss, "color": random_color,
                        "color_cosine_similarity": cosine_similarity_value,
                        "theta_cosine_similarity": theta_cosine_similarity})
        print(
        ...
        # Save the results
        torch.save(results, "results/exp2_flip_label.pth")
